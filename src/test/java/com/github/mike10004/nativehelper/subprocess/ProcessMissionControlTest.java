...
public class ProcessMissionControlTest {
    ...
    public void launch_ProcessOutputControls_memory() throws Exception {
        ...
        StreamContext<BucketContext, StreamInput, StreamInput> ctrl = (StreamContext<BucketContext, StreamInput, StreamInput>) StreamContexts.memoryByteSources(null);
        StreamControl outputcontext = ctrl.produceControl();
        Execution<StreamInput, StreamInput> execution = executor.launch(outputcontext, c -> {
                    StreamContent<StreamInput, StreamInput> xformed = ctrl.transform(c, (BucketContext) outputcontext);
